...
def populate_main_window(frm_main):
    ...
    lbl_length = Label(frm_main, text="Pendulum Length:")

    # Create an integer entry box where the user will enter her length.
    ent_length = FloatEntry(frm_main, width=4, lower_bound=0, upper_bound=999)

    # Create a label that displays "years"
    lbl_length_units = Label(frm_main, text="meters")

    # Create a label that displays "Rates:"
    lbl_rates = Label(frm_main, text="Time:")

    # Create labels that will display the results.
    lbl_time = Label(frm_main, width=3)
    lbl_rate_units = Label(frm_main, text="seconds")
    ...
    lbl_time.grid(      row=1, column=1, padx=3, pady=3)
    ...
    def calculate(event):
        ...
        """
        try:
            # Get the user's length.
            pendulum_length = ent_length.get()

            swing_time = 2 * math.pi * math.sqrt(pendulum_length / 9.81)

            # Display the slowest and fastest benficial
            # heart rates for the user to see.
            lbl_time.config(text=f"{swing_time:.2f}")
